...
export default class GameManager extends cc.Component {
    ...
    onLoad() {
        ...
        // Cap to the 960x640 Canvas design resolution so the HUD + play area stay
        // inside the always-visible region. (getVisibleSize can over-report the width
        // right after a loadScene, which pushed the corner HUD off-screen.)
        this.DESIGN_W = Math.min((vs && vs.width)  ? vs.width  : 960, 960);
        this.DESIGN_H = Math.min((vs && vs.height) ? vs.height : 640, 640);
        ...
        this.buildPlayer();
        this.buildHUD();
        this.layoutHUD();            // pin HUD to the real visible edges (never off-screen)
        ...
        this.node.on(cc.Node.EventType.TOUCH_START, () => {
            if (this.phase === "ready") this.startGame();   // 'over' exits to the overworld via endGame()'s scheduled return
        }, this);
    }
    ...
    // last seen visible size — used to re-pin the HUD whenever the window resizes
    private _lastVW = 0;
    private _lastVH = 0;
    ...
    private buildDangerLine() {
        this.dangerNode.addComponent(cc.Graphics);
        this.drawDangerLine();
    }

    // วาด/วาดใหม่เส้นอันตรายให้กว้างเต็มพื้นที่ที่มองเห็นจริง (กันมีช่องว่างขอบจอกว้าง)
    private drawDangerLine() {
        const g = this.dangerNode ? this.dangerNode.getComponent(cc.Graphics) : null;
        if (!g) return;
        g.clear();
        const y = this.dangerY;
        const vs = cc.view.getVisibleSize();
        const w = Math.max(this.HALF_W, (vs && vs.width ? vs.width : 0) / 2) + 80;
        // แถบเรืองแสงจาง ๆ
    ...
    // ---------- จัดวาง HUD ให้ติดขอบจอจริงเสมอ (กันหลุดจอทุกอัตราส่วน) ----------
    // โค้ดเป็นเจ้าของการจัดวาง HUD เอง โดยอิง "ขนาดที่มองเห็นจริง"
    // (cc.view.getVisibleSize) ไม่ใช่ design width ที่ถูก cap ไว้ — Label ที่ลาก
    // วางใน editor ถูกจัดสำหรับ canvas เดิม 1280x720 จึงหลุดออกนอกพื้นที่ 960x640
    // เราจึง "รับเลี้ยง" Label เหล่านั้นเข้ามาไว้ในเฟรมเดียว (uiLayer) แล้วปักหมุดที่มุมจอ
    private layoutHUD() {
        const vs = cc.view.getVisibleSize();
        const halfW = (vs && vs.width  ? vs.width  : this.DESIGN_W) / 2;
        const halfH = (vs && vs.height ? vs.height : this.DESIGN_H) / 2;
        const M = 26;   // ระยะห่างจากขอบจอ

        const place = (comp: cc.Label | cc.RichText, ax: number, ay: number, x: number, y: number) => {
            if (!comp || !comp.node) return;
            // รับ Label ที่ลากผูกมาจาก editor เข้ามาไว้ในเฟรมพิกัดเดียว (uiLayer ที่อยู่กึ่งกลาง Canvas)
            if (comp.node.parent !== this.uiLayer) comp.node.parent = this.uiLayer;
            comp.node.anchorX = ax;
            comp.node.anchorY = ay;
            comp.node.setPosition(x, y);
        };

        // มุมซ้ายบน: SCORE แล้วตามด้วย COMBO
        place(this.scoreLabel, 0, 1, -halfW + M, halfH - M);
        place(this.comboLabel, 0, 1, -halfW + M, halfH - M - 46);
        // มุมขวาบน: หัวใจ/ชีวิต แล้วตามด้วย WPM
        place(this.livesLabel, 1, 1, halfW - M, halfH - M);
        place(this.wpmLabel,   1, 1, halfW - M, halfH - M - 46);
        // กึ่งกลางบน: LEVEL + ความคืบหน้า
        place(this.levelLabel,   0.5, 1, 0, halfH - M);
        place(this.progressInfo, 0.5, 1, 0, halfH - M - 40);
        if (this.progressBar) {
            if (this.progressBar.parent !== this.uiLayer) this.progressBar.parent = this.uiLayer;
            this.progressBar.setPosition(0, halfH - M - 66);
        }
        // ช่องคำที่กำลังพิมพ์ (ล่างจอ กึ่งกลาง — ปลอดภัยอยู่แล้ว)
        if (this.inputLabel && this.inputLabel.node) {
            if (this.inputLabel.node.parent !== this.uiLayer) this.inputLabel.node.parent = this.uiLayer;
            this.inputLabel.node.anchorX = 0.5;
            this.inputLabel.node.anchorY = 0.5;
            this.inputLabel.node.setPosition(0, this.playerY + 96);
        }
    }
    ...
    // วนลูปเกม
    update(dt: number) {
        // Keep the HUD + danger line pinned to the REAL visible edges on any
        // window resize — and fix the first-frame layout, because getVisibleSize()
        // can over-report the width right after a loadScene. Runs every phase.
        const vs = cc.view.getVisibleSize();
        if (vs && (vs.width !== this._lastVW || vs.height !== this._lastVH)) {
            this._lastVW = vs.width;
            this._lastVH = vs.height;
            this.layoutHUD();
            this.drawDangerLine();
            if (this._centerBg && this._centerBg.active) this._drawCenterBg();
        }
    ...
    private startGame() {
        // Don't allow a restart while a scene-return is pending (typing loss).
        // endGame() schedules loadScene('MainScene') after 2.8s; an ENTER/tap
        // during that window would restart the game in place, but the queued
        // loadScene would still fire and yank the player out mid-restart.
    ...
    private levelCleared() {
        ...
        if (isLast) {
            this.showCenter(`ALL 5 LEVELS CLEAR!  🎉  PASSED!\n\nWPM  ${wpm}     SCORE  ${this.score}\n\nBONUS ROUND — keep typing for extra points!\n(your win is already locked in — last until you\nrun out of lives to bank your final score)`);
        } else {
    ...
    // Story integration: clearing all 5 levels = PASS the Professor challenge.
    // The win is locked in here (this._passed), but the game does NOT end — it rolls
    // into an endless BONUS round (x2 score) so the player can pile up leaderboard
    // points. endGame() later banks the win + the full accumulated score.
    private enterBonus() {
        this._passed = true;
        this.isBonus = true;
        this.lives = this.LIVES_MAX;   // fresh hearts for the bonus run
        this.combo = 0;
        this.cleared = 0;
        this.curSpawn = 1.25;
        this.curSpeed = 88;
        ...
        this.phase = "playing";
        this.setHudVisible(true);
        this.playBgm(this.bonusBgmClip || this.bgmClip);   // upbeat bonus track
        this.updateLevel();
        this.updateProgress();
        this.updateLives();
        this.updateCombo();
    ...
    private _passed = false;   // cleared all 5 levels → professor challenge already won

    private endGame() {
        this.phase = "over";
        this._returning = true;   // lock out in-place restart during the scene-return window
        this.target = null;
        this.setHudVisible(false);
        // If the player already cleared all 5 levels, running out of lives in the
        // BONUS round is still a WIN — bank it with the full accumulated score.
        const won = this._passed;
        if (won) {
            this.showCenter(
                `BONUS OVER!  🎉\n\nYou cleared all 5 levels — challenge PASSED!\nFINAL SCORE  ${this.score}     WPM  ${this.currentWPM()}\nBEST COMBO  ${this.bestCombo}\n\nProf. Hung is impressed — heading back...`
            );
        } else {
            this.showCenter(
                `GAME OVER\n\nReached:  LEVEL ${this.level + 1}\nSCORE  ${this.score}     WPM  ${this.currentWPM()}\nBEST COMBO  ${this.bestCombo}\n\nHeading back to Prof. Hung...`
            );
        }
        const StoryState: any = require("StoryState");
        StoryState.lastResult = { key: "professor", win: won, score: this.score };
        this.scheduleOnce(() => cc.director.loadScene("MainScene"), 2.8);
    }
    ...
    // วาด/วาดใหม่กล่องพื้นหลังข้อความกลางจอ ให้พอดีพื้นที่ที่มองเห็นจริง
    // (กันกล่อง 900px ล้นขอบจอบนหน้าต่างแคบ ๆ)
    private _drawCenterBg() {
        if (!this._centerBg) return;
        const g = this._centerBg.getComponent(cc.Graphics);
        if (!g) return;
        g.clear();
        const vs = cc.view.getVisibleSize();
        const BW = Math.min(900, (vs && vs.width  ? vs.width  : 960) - 32);
        const BH = Math.min(560, (vs && vs.height ? vs.height : 640) - 32);
    ...
    private showCenter(msg: string) {
        ...
        if (this._centerBg) { this._centerBg.active = true; this._drawCenterBg(); }
