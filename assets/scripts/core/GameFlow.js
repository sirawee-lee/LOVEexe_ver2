...
const StoryState = require('StoryState');   // shared with the overworld (persists across loadScene)
...
const GameFlow = {
    ...
    _goToEnding() {
        // Boss cleared → record it and return to the overworld. GameController
        // plays Mei's finale dialogue (mei_after) when MainScene reloads.
    ...
    lose() {
        ...
        if (this.love <= 0) {
            this._showOverlay({
                title: 'GAME OVER',
                titleColor: COL_LOSE,
                subtitle: 'Out of love... go find Mei to try again.',
                btnText: 'Back',
                btnColor: COL_BTN2,
                onClick: () => { this.started = false; cc.director.loadScene('MainScene'); },
            });
        } else {
